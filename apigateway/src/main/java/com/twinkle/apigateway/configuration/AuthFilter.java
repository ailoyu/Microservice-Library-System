...
public class AuthFilter extends AbstractGatewayFilterFactory<AuthFilter.Config> {
	...
	public GatewayFilter apply(Config config) {
		...
		return (exchange, chain) -> {
  		...
  		DecodedJWT decodedJWT = verifier.verify(token[1]); // giải mải token dựa vào verify đó
  		String username = decodedJWT.getSubject(); // lấy subject truyền username từ trc
       if(username == "" || username == null) {
    	   throw new RuntimeException("Athorization error");
       }
       // Cho request của user vào service/api nó muốn 
    	   ServerHttpRequest request = exchange.getRequest().mutate().
    	      header("X-auth-username", username). // cho header là username
    	      build();
